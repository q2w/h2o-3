...
testPartialPlots <- function() {
  ...
  h2o_pp_weight_NA = h2o.partialPlot(object = prostate_gbm, data = prostate_hex, cols = c("AGE", "RACE"), plot = F, weight_column_index="constWeight", add_missing_NA=TRUE)
  h2o_pp_vweight_NA = h2o.partialPlot(object = prostate_gbm, data = prostate_hex, cols = c("AGE", "RACE"), plot = F, weight_column_index="variWeight", add_missing_NA=TRUE)
