...
function cleanup () {
  ...
  if [ $RC -ne 0 ]; then
    cat $OUTDIR/out.0
    echo h2o-persist-http junit tests FAILED
  else
    echo h2o-persist-http junit tests PASSED
  fi
  exit $RC
}
...
# On this h2o-persist-http testMultiNode.sh only, force the tests.txt to be in the same order for all machines.
